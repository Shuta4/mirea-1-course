...
def main():
    ...
    plt.xlim(data[YEAR].max(), data[YEAR].min())
    anamorphose_x1 = data.iloc[5][YEAR]
    anamorphose_x2 = data.iloc[-1][YEAR]
    anamorphose_y1 = data.iloc[5][LN_PROD_DIV_SUM]
    anamorphose_y2 = data.iloc[-1][LN_PROD_DIV_SUM]
    anamorphose_k = math.atan(abs(anamorphose_y1 - anamorphose_y2) /
                    abs(anamorphose_x2 - anamorphose_x1))

    plt.plot([anamorphose_x1, anamorphose_x2],
             [anamorphose_y1, anamorphose_y2], color='orange')
    make_horizontal(data[YEAR].min(), data[YEAR].max(),
                    log(anamorphose_k), text="ln(k)", color='red')
